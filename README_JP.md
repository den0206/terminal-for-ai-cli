...
<p align="center">
  <!-- BEGIN:release -->
  最新リリース: <a href="https://github.com/den0206/terminal-for-ai-cli/releases/tag/Ver_0.3.0"><strong>Ver_0.3.0</strong></a>（2026-09-03）
  <!-- END:release -->
</p>
...
- **画像・動画を選択** — 各ペインの `📎` から選択したファイルの実パスを、そのターミナルへ入力。
...
各ターミナルペインのタイトルバーにある `📎` を押すと、OS 標準のファイル選択画面から画像・動画を複数選択できます。
ファイル自体はコピーせず、ボタンを押したターミナルへ、選択順にシェルエスケープ済みの実パスを空白区切りで入力します。
対応形式は PNG、JPEG、GIF、WebP、BMP、TIFF、HEIC、SVG、MP4、MOV、M4V、WebM、AVI、MKV です。

パスだけを入力するため、ファイルサイズと選択件数には上限がありません。安全策として、エスケープ後の入力全体が
64 KiB を超える選択は一件も入力せず拒否します。Remote ワークスペースを含め、ターミナルと同じファイルシステムを
選択対象とし、選択完了またはキャンセル後は元のターミナルへフォーカスを戻します。

### ファイルと画像のドロップ

ドラッグする際は **`Shift` を押したまま** ドロップします。押さない場合はエディタ側のドロップ処理が
優先されるため、これは意図的な仕様です。シェルに入力される内容はドラッグ元によって変わります。

| ドラッグ元 | 入力されるもの | 理由 |
|------------|----------------|------|
| エディタの**エクスプローラ**（任意のファイル） | そのファイル自身のエスケープ済みパス | ドロップに `text/uri-list` が付き、実パスが分かるため |
| **OS**（Finder / エクスプローラ）、画像のみ | このウィンドウのストレージに保存したコピーのパス | Webview は OS からドロップされたファイルの実パスを読めないため、バイト列をコピーする |

コピーはスクリーンショットには妥当ですが、ソースファイルには不適切です（エージェントがコピーを
編集しても元のファイルは変わりません）。そのため OS からのドロップは従来どおり `image/*` の 10MB 以下だけを
受け付けます。ソースファイルは実パスの分かるエクスプローラからドラッグしてください。

1 回のドロップで入力されるのは最大 50 パスまでで、空白区切りです。使われるのは `file:` の URI だけで、
そのプラットフォームのシェルで安全に引用できないパスは、中途半端にエスケープせずスキップします。

### リンクを開く

出力中の `http(s)` URL を `Cmd`（macOS）/ `Ctrl`（Windows, Linux）+ クリックすると、既定のブラウザで開きます。
VS Code のターミナルと同じ操作です。認証 URL を表示する AI CLI のサインインで便利です。

**プレーンクリック**では、リンクの近くに小さなポップオーバーが出て「**ブラウザで開く**」と
「**URL をコピー**」を選べます。コピーはサインインで効きます。表示された URL を、目的のアカウントで
既にサインイン済みのブラウザに貼りたいことが多いためです。ポップオーバーが出るのは、選択範囲が無い状態で
リンク上を動かさずにクリックしたときだけなので、リンクをまたぐ範囲選択はこれまでどおりです。
`Esc` か別の場所のクリックで閉じ、その `Esc` は CLI には送られません。
...
`Cmd` / `Ctrl` + `F` でターミナルの上に検索バーが開きます。対象は**フォーカス中のターミナルだけ**で、
テーマのドロップダウンと同じ考え方です。左のバッジがどちらを検索しているかを示します。

| キー | 動作 |
|------|------|
| `Cmd` / `Ctrl` + `F` | 検索バーを開く（既に入力があれば選択状態にする） |
| `Enter` | 次の一致へ |
| `Shift` + `Enter` | 前の一致へ |
| `Esc` | 閉じてターミナルにフォーカスを戻す |

`Aa` で大文字小文字を区別し、`.*` で正規表現として扱います。一致箇所はすべてハイライトされ、
件数が入力欄の隣に出ます。バーを閉じるとハイライトも消えます。`Ctrl` + `F` はシェルより先に
検索バーが受け取るため、CLI に前方移動キーとして流れることはありません。
...
各ターミナルのスクロールバックは、出力が落ち着いたタイミングでこのウィンドウのストレージに書き出され、
エディタを起動し直したときに読み戻されます。戻ってくるのは**履歴だけ**です。そこに写っているシェルは
前のプロセスと一緒に終了しているため、復元されたテキストは前後を薄い区切り線で囲んで示されます。
その下では新しいシェルが通常どおり起動します。

保存単位はテーマと同じくターミナル番号（`Terminal 1` / `Terminal 2`）です。セッション ID は起動ごとに
変わるためです。「Clear all sessions」、設定をオフにしたとき、そして起動時に 24 時間（保存画像と同じ TTL）を過ぎたものが削除されます。
ディスクに何も残したくない場合は `aiTerminal.restoreScrollback` を `false` にしてください。

### 描画

GPU が使える環境では、ターミナルを **WebGL レンダラ**で描画します。xterm.js が既定で使う
DOM レンダラと比べて変わるのは次の 2 点です。

- **罫線・ブロック文字をフォントではなくターミナル自身が描く**ため、AI CLI の TUI が引く枠線が
  この行間でも途切れずに繋がります。
- **大量出力時の CPU 負荷が下がります**。行ごとに DOM ノードを作らず、GPU のテクスチャアトラスから
  グリフを描くためです。

`aiTerminal.rendererType` で何を試すかを決めます。

| 値 | 動作 |
|----|------|
| `auto`（既定） | WebGL を試し、初期化できなければ DOM レンダラに切り替える |
| `webgl` | 同じ動作を明示的に指定する（失敗時はやはり DOM レンダラに切り替わります） |
| `dom` | WebGL を使わない |

このフォールバックは GPU が無い環境のためだけのものではありません。WebGL のコンテキストは実行中にも
失われることがあり（ドライバのリセット、1 ウィンドウ内のコンテキスト数の上限など）、このビューは
非表示の間もコンテキストを保持し続けます。失われた場合はアドオンを外し、そのセッションの間は両ペインとも
DOM レンダラで描画を続けます。再取得を試みて画面が点滅するより確実だからです。
...
| 画像・動画の選択 | 各ペインの `📎` からメディアを選び、シェルエスケープ済みの実パスをそのターミナルへ入力 |
| ファイルのドラッグ&ドロップ | `Shift` + ドロップ → エクスプローラからはファイル自身のパス、OS からの画像は保存したコピーのパスを入力 |
| リンクのクリック | `Cmd` / `Ctrl` + クリックで `http(s)` URL を既定のブラウザで開く。プレーンクリックでは「開く / コピー」を選べる |
| 画像クリーンアップ | セッション終了時・拡張の deactivate 時・起動時（孤児と 24 時間 TTL）に削除 |
| 使用状況表示 | ディスク上の合計（保存画像 + 保存スクロールバック）と拡張ホストの RSS。可視時に 30 秒ごと更新 |
| スクロールバックの復元 | 各ターミナルのスクロールバックがエディタ再起動をまたいで残る（読み取り専用の履歴として） |
| スクロールバック内検索 | `Cmd` / `Ctrl` + `F` でフォーカス中のターミナルを検索。件数表示・大文字小文字・正規表現トグル付き |
...
| GPU 描画 | GPU が使える環境では WebGL レンダラ。使えなければ自動的に DOM レンダラへ切り替え |
...
| リソース表示 | `aiTerminal.showResourceStats` | この拡張がディスクに置いているサイズ（保存画像 + 保存スクロールバック）と拡張ホストのメモリをツールバーに表示します。既定は `true`。`false` にすると表示だけでなく背後のポーリングも止まります。 |
| スクロールバックの復元 | `aiTerminal.restoreScrollback` | 各ターミナルのスクロールバックをディスクに保存し、再起動後に読み戻します。既定は `true`。**machine スコープ**。 |
| 描画方式 | `aiTerminal.rendererType` | ターミナルの描画方式。`auto`（既定）/ `webgl` / `dom`。[描画](#描画) を参照。 |
...
- **保存されるスクロールバック** — `aiTerminal.restoreScrollback` が有効なとき、ターミナルの出力は
  このウィンドウのストレージに書き出されます。CLI が表示したもの（トークンを含む）が削除されるまで
  ディスク上に残るということです。ワークスペースストレージ内に留まり、マシンの外には出ません。
  「Clear all sessions」、設定のオフ、24 時間経過のいずれかで削除されます。読み戻したスナップショットは
  検証とサイズ上限のチェックを通してからターミナルに書き込まれます。
...
| シェル自体は再起動をまたげない | スクロールバックは読み取り専用の履歴として戻りますが、OS プロセスは終了しています。再起動後は必ず新しいシェルになります。 |
...
**`Shift`** を押しながらドラッグし、アクティブなセッションがあることを確認してください
（ドロップはアクティブセッションに送られます）。OS から直接のドロップは `image/*` かつ 10MB 以下だけを
受け付けます。それ以外は、実パスを運べるエディタのエクスプローラからドラッグしてください。
